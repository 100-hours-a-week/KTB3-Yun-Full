import handler.InputExceptionHandler;
...
public enum ProgramState {
    ...
    남은 고민은 main과 ProgramState에 ProgramContext 객체가 중복으로(?) 넘겨지는 구조를 어떻게 리팩토링할까입니다. (09.19)
     */
    ONBOARDING{
        @Override
        public ProgramState handle(ProgramContext context){
            bookService.programOnboarding();
            String userChoice = bookService.input.next();
            int menuNum = exceptionHandler.convertToInteger(userChoice);
            switch (menuNum) {
                case 1:
                    return LIST;
                case 2:
    ...
    DETAIL{
        ...
        public ProgramState handle(ProgramContext context){
            ...
            String bookId = bookService.input.next();
            int bookNum = exceptionHandler.convertToInteger(bookId);
            if (bookNum > ProgramContext.bookList.length || bookNum < 1) {
                    System.out.println("올바른 번호를 입력해주세요.");
                    return DETAIL;
                }
            bookService.showBookDetails(bookNum);
            System.out.println("-------------------------------------------------");
    ...
    public static final BookService bookService = new BookService(new ProgramContext());
    public static final InputExceptionHandler exceptionHandler = new InputExceptionHandler();
}
